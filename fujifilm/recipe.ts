import { parseFujifilmMakerNote } from "./makerNoteParser";
...
export type WeakStrong = "off" | "weak" | "strong";
...
export const processWeakStrong = (value: number): WeakStrong => {
  console.log("processWeakStrong raw value:", value);
  switch (value) {
    case 32: // 8에서 32로 변경
      return "weak";
    case 64: // 16에서 64로 변경
  ...
  parseFujifilmMakerNote(bytes, (tag, numbers) => {
    switch (tag) {
      ...
      case TAG_ID_COLOR_CHROME_EFFECT:
        recipe.colorChromeEffect = processColorChrome(numbers[0]);
        break;
      case TAG_ID_COLOR_CHROME_FX_BLUE:
        recipe.colorChromeFXBlue = processColorChrome(numbers[0]);
        break;
